...
template ModularExponentiation(nBits) {
    ...
    for (var i = 0; i < nBits; i++) {
        applyMultiplier[i] <-- exp_bits[i] == 1 ? currentBases[i] : 1;
...
template RabinMillerPrimalityTest(k, nBits, maxRounds, CHUNK_SIZE, CHUNK_NUMBER) {
    signal input n[CHUNK_NUMBER]; // test big number
    signal input a[k][CHUNK_NUMBER]; // prime number set, a < 2**64
    signal input d_bits[nBits]; // d * 2^r = n - 1 

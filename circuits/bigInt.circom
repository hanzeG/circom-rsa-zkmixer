...
template PrimeReduce(CHUNK_SIZE, CHUNK_NUMBER, M, P, M_OUT){
    ...
    for (var i = 0; i < M; i++){
        for (var j = 0; j < CHUNK_NUMBER; j++){
            out_sum[j] += in[i + CHUNK_NUMBER] * r[i][j]; 
        }
    }
    for (var i = 0; i < CHUNK_NUMBER; i++){
        out[i] <== out_sum[i];
        ...
        for (var j = 0; j < 2 * CHUNK_NUMBER - 1; j++) {
            ...
            for (var deg1 = 0; deg1 < l; deg1++) {
                for (var deg2 = 0; deg2 < CHUNK_NUMBER; deg2++) {
                    aPoly[i][j] = aPoly[i][j] + a[deg1][deg2] * pow[i][deg1] * pow[j][deg2]; 
                    bPoly[i][j] = bPoly[i][j] + b[deg1][deg2] * pow[i][deg1] * pow[j][deg2]; 
                }
            }
            for (var deg1 = 0; deg1 < 2 * l - 1; deg1++) {
                for (var deg2 = 0; deg2 < 2 * CHUNK_NUMBER - 1; deg2++) {
                    outPoly[i][j] = outPoly[i][j] + out[deg1][deg2] * pow[i][deg1] * pow[j][deg2]; 
                }
            }
        }
    }

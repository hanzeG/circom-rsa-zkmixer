...
contract MerkleTreeWithHistory {
    ...
    constructor(uint32 _levels, address _poseidon2Contract) {
    ...
    /**
    @dev Hash 2 tree leaves, returns Poseidon2(_left, _right)
  */
    function callPoseidon2Yul(
        bytes32 input1,
        bytes32 input2
    ) public returns (bytes32) {
        // Call the contract's fallback function
        (bool success, bytes memory result) = poseidon2Contract.call(
            abi.encode(input1, input2)
        );
        require(success, "poseidon2 call failed");
        return bytes32(result); // Return the raw result
    }
    ...
    function _insert(bytes32 _leaf) internal returns (uint32 index) {
        ...
        for (uint32 i = 0; i < levels; i++) {
            ...
            currentLevelHash = callPoseidon2Yul(left, right);
            currentIndex /= 2;
        }

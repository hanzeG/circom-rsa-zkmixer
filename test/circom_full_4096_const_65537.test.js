...
const chunk_size = 64;
const chunk_num = 64;
const security_bit = 2048;

const mt_zero = 513;
const mt_depth = 20;

describe("Test register_4096.circom: register with secret prime p, q to generate N", function () {
    ...
    before(async function () {
        circuit = await wasm_tester(path.join(__dirname, "circuits", "register_4096.circom"));
    ...
    it("should register successfully", async function () {
        ...
        const outputPath = path.join(__dirname, "../circuit_input/register_4096.json");
...
describe("Test ex_transfer_4096.circom: Alice external transfer UTXO to Bob", function () {
    this.timeout(1000 * 1000);
    ...
    before(async function () {
        circuit = await wasm_tester(path.join(__dirname, "circuits", "ex_transfer_4096.circom"));
    ...
    it("should external transfer successfully", async function () {
        ...
        // Save the input object to a JSON file at relative path "../circuit_input"
        const outputPath = path.join(__dirname, "../circuit_input/ex_transfer_4096.json");
        fs.writeFileSync(outputPath, JSON.stringify(input, null, 2));
...
describe("Test spend_4096.circom: Bob spend UTXO from Alice", function () {
    this.timeout(1000 * 1000);
    ...
    before(async function () {
        circuit = await wasm_tester(path.join(__dirname, "circuits", "spend_4096.circom"));
        mt = await MT.build(mt_zero, mt_depth);
        alice = await USER.register(security_bit, chunk_size, chunk_num);
        bob = await USER.register(security_bit, chunk_size, chunk_num);
    });
    ...
    it("should spend UTXO successfully", async function () {
        ...
        // Save the input object to a JSON file at relative path "../circuit_input"
        const outputPath = path.join(__dirname, "../circuit_input/spend_4096.json");
        fs.writeFileSync(outputPath, JSON.stringify(input, null, 2));
...
describe("Test in_transfer_4096.circom: Bob internal transfer to Charles", function () {
    this.timeout(1000 * 1000);
    ...
    before(async function () {
        circuit = await wasm_tester(path.join(__dirname, "circuits", "in_transfer_4096.circom"));
        mt = await MT.build(mt_zero, mt_depth);
        alice = await USER.register(security_bit, chunk_size, chunk_num);
        bob = await USER.register(security_bit, chunk_size, chunk_num);
    ...
    it("should internal transfer UTXO successfully", async function () {
        ...
        // Save the input object to a JSON file at relative path "../circuit_input"
        const outputPath = path.join(__dirname, "../circuit_input/in_transfer_4096.json");
        fs.writeFileSync(outputPath, JSON.stringify(input, null, 2));
